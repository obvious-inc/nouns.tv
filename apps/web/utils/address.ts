...
function shortenString(str: string, length: number = 3) {
  return (
    str.substring(0, length + 2) + "..." + str.substring(str.length - length)
  );
}

export function shortenAddress(_address: string, length: number = 3) {
  const address = getAddress(_address);
  return shortenString(address, length);
}

export function shortenTx(txHash: string, length: number = 3) {
